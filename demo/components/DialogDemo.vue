...
const simpleOpened = ref(false)
const fullOpened = ref(false)
const overflowOpened = ref(false)
const formOpened = ref(false)
const destructiveOpened = ref(false)
const nonModalOpened = ref(false)

const reportClosingReason = (reason: UiDialogClosingReason) => {
  if (reason.cancelled) {
...
const openSimple = () => simpleOpened.value = true
const basicClosed = (e: CustomEvent<UiDialogClosingReason>) => {
  simpleOpened.value = false
  reportClosingReason(e.detail)
}

const openFull = () => fullOpened.value = true
const fullClosed = (e: CustomEvent<UiDialogClosingReason>) => {
  fullOpened.value = false
  reportClosingReason(e.detail)
}

const openOverflow = () => overflowOpened.value = true
const overflowClosed = (e: CustomEvent<UiDialogClosingReason>) => {
  overflowOpened.value = false
  reportClosingReason(e.detail)
}

const openForm = () => formOpened.value = true
const formClosed = (e: CustomEvent<UiDialogClosingReason>) => {
  formOpened.value = false
  reportClosingReason(e.detail)
}

const openDestructive = () => destructiveOpened.value = true
const destructiveClosed = (e: CustomEvent<UiDialogClosingReason>) => {
  destructiveOpened.value = false
  reportClosingReason(e.detail)
}

const openNonModal = () => nonModalOpened.value = true
const nonModalClosed = (e: CustomEvent<UiDialogClosingReason>) => {
  nonModalOpened.value = false
  reportClosingReason(e.detail)
}

let imperativeDialog: HTMLElement | null = null

const openImperative = () => {
  if (imperativeDialog) {
    document.body.removeChild(imperativeDialog)
    imperativeDialog = null
  }
  const dialog = document.createElement('ui-dialog') as any
  dialog.textContent = 'This is a dialog content.'
  dialog.dismissLabel = 'Close'
  dialog.confirmLabel = 'OK'
  dialog.open = true
  document.body.appendChild(dialog)
  imperativeDialog = dialog
  dialog.addEventListener('close', (e: Event) => {
    document.body.removeChild(dialog)
    imperativeDialog = null
    const event = e as CustomEvent<UiDialogClosingReason>
    reportClosingReason(event.detail)
  })
}

const handleInertButton = () => {
...
<template>
  <div class="demo-page">
    <section class="demo-section">
      ...
      <ui-button color="filled" @click="openSimple">Open</ui-button>
      <ui-dialog :open="simpleOpened" @close="basicClosed">
    ...
    <section class="demo-section">
      ...
      <ui-dialog :open="fullOpened" @close="fullClosed" modal style="--ui-dialog-max-width: 380px">
        ...
        <ui-divider></ui-divider>
        <ui-button color="text" slot="button" @click="handleInertButton">Learn more</ui-button>
    ...
    <section class="demo-section">
      ...
      <ui-dialog :open="overflowOpened" @close="overflowClosed" modal style="--ui-dialog-max-width: 400px; --ui-dialog-max-height: 500px;">
        ...
        <div style="overflow-y: auto; max-height: 300px;">
          ...
          <p>Welcome to our application. These terms and conditions outline the rules and regulations for the use of our software.</p>
          <h3>2. User Agreement</h3>
          <p>By accessing this application, we assume you accept these terms and conditions. Do not continue to use our application if you do not agree to take all of the terms and conditions stated on this page.</p>
          ...
          <p>Users are responsible for maintaining the confidentiality of their account information and for all activities that occur under their account. You agree to:</p>
          <ul>
          ...
          <h3>9. Disclaimer</h3>
          <p>The information on this application is provided on an "as is" basis. To the fullest extent permitted by law, this company excludes all representations, warranties, conditions and terms.</p>
          ...
          <p>If you have any questions about these Terms and Conditions, please contact us at support@example.com or through our contact form.</p>
        </div>
        ...
        <ui-dialog :open="formOpened" @close="formClosed" modal style="--ui-dialog-max-width: 500px;" submitClose>
          ...
          <div style="display: flex; flex-direction: column; gap: 16px;">
            <label style="display: flex; flex-direction: column; gap: 4px;">
              ...
              <input type="text" name="fullName" required style="padding: 12px; border: 1px solid var(--md-sys-color-outline); border-radius: 4px; font-size: 14px;" placeholder="Enter your full name" />
            </label>
            <label style="display: flex; flex-direction: column; gap: 4px;">
              <span style="font-weight: 500; color: var(--md-sys-color-on-surface);">Email *</span>
              <input type="email" name="email" required style="padding: 12px; border: 1px solid var(--md-sys-color-outline); border-radius: 4px; font-size: 14px;" placeholder="Enter your email address" />
            </label>
            <label style="display: flex; flex-direction: column; gap: 4px;">
              <span style="font-weight: 500; color: var(--md-sys-color-on-surface);">Phone Number</span>
              <input type="tel" name="phone" style="padding: 12px; border: 1px solid var(--md-sys-color-outline); border-radius: 4px; font-size: 14px;" placeholder="Enter your phone number (optional)" />
            </label>
            <label style="display: flex; flex-direction: column; gap: 4px;">
              <span style="font-weight: 500; color: var(--md-sys-color-on-surface);">Department</span>
              <select name="department" style="padding: 12px; border: 1px solid var(--md-sys-color-outline); border-radius: 4px; font-size: 14px; background: white;">
            ...
            <label style="display: flex; align-items: center; gap: 8px; margin-top: 8px;">
              ...
              <span style="font-size: 14px; color: var(--md-sys-color-on-surface);">Subscribe to our newsletter</span>
            </label>
            <label style="display: flex; align-items: center; gap: 8px;">
              <input type="checkbox" name="terms" required style="margin: 0;" />
              <span style="font-size: 14px; color: var(--md-sys-color-on-surface);">I agree to the <a href="#" style="color: var(--md-sys-color-primary);">Terms and Conditions</a> *</span>
            </label>
          </div>
          <ui-button color="text" slot="button" value="dismiss">Cancel</ui-button>
    ...
    <section class="demo-section">
      ...
      <ui-button color="filled" @click="openNonModal">Open non-modal dialog</ui-button>
      <ui-dialog :open="nonModalOpened" @close="nonModalClosed" confirmLabel="Got it" dismissLabel="Close">
        <ui-icon slot="icon" icon="info"></ui-icon>
        <span slot="title">Non-Modal Information</span>
        <p>This is a non-modal dialog. Unlike modal dialogs, you can still interact with the content behind this dialog. The dialog is automatically centered in the viewport and maintains the smooth opening animation.</p>
        <p>Non-modal dialogs are useful for displaying information that doesn't require the user's full attention or when you want to allow interaction with the background content.</p>
      </ui-dialog>
    </section>
    ...
    <section class="demo-section">
      ...
      <ui-button color="filled" @click="openImperative">Open</ui-button>
    </section>
  </div>
</template>
